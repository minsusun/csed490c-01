...
string name = "CUDA(matrix distance calculation/thrust sort)";
...
int main(int argc, char *argv[]) {
  ...
  for(int i = 0; i < N; i++) thrust::sort_by_key(thrust::device, dD + i * N, dD + (i + 1) * N, dIdxMat + i * N);
