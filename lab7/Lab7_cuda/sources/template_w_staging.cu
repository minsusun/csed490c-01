...
__global__ void SpMV_JDS_T(int numRows, int numColumns, float *data, int *col_idx, int *col_ptr, int *row_idx, float *B, float *C, int len) {
...
int main(int argc, char **argv) {
  ...
  SpMV_JDS_T<<<dimGrid, dimBlock, numBRows * numBColumns * sizeof(float)>>>(numARows, numAColumns, device_data, device_col_idx, device_col_ptr, device_row_idx, deviceB, deviceC, (numAColumns - 1)/THREADS_PER_BLOCK + 1);
